...
const EditProductPage = () => {
  ...
  const handleImages = async (images: (File | string)[]): Promise<string[]> => {
    ...
    return [...uploaded, ...oldFiles];
  };
  const onSubmit: SubmitHandler<CreateProductFormValues> = async (data) => {
    try {
      const images = data.images ? await handleImages(data.images) : [];
      const { message, data: product } = await mutateAsync({
        id: Number(id),
        data: { ...data, images },
      });
      showSuccess(message);
  ...
  watch((value, { name }) => {
    ...
    if (name === "discountType") {
      ...
      } else if (value.discountType === productData.data.discountType) {
        setValue("discountValue", productData.data.discountValue);
      } else {
